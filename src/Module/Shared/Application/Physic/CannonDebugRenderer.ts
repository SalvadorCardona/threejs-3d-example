...
export default class CannonDebugRenderer {
  ...
  private _particleGeometry: Geometry
  ...
  constructor(scene: THREE.Scene, world: CANNON.World, options?: object) {
    ...
    this._particleGeometry = new Geometry()
  ...
  private _typeMatch(mesh: THREE.Mesh | THREE.Points, shape: CANNON.Shape): Boolean {
    ...
    const geo: Geometry | THREE.BufferGeometry = mesh.geometry
    return (
...
export function createTrimesh(
  geometry: Geometry | THREE.BufferGeometry
): CANNON.Trimesh {
  if (!(geometry as THREE.BufferGeometry).attributes) {
    geometry = new THREE.BufferGeometry().fromGeometry(geometry as Geometry)
  }
  const vertices: number[] = <number[]>(
    (geometry as THREE.BufferGeometry).attributes.position.array
  )
...
export function createConvexPolyhedron(
  ...
  if (!(geometry as Geometry).vertices) {
    geometry = new Geometry().fromBufferGeometry(geometry as THREE.BufferGeometry)
    geometry.mergeVertices()
    geometry.computeBoundingSphere()
    geometry.computeFaceNormals()
  }
  const points: CANNON.Vec3[] = (<Geometry>geometry).vertices.map(function (v) {
    return new CANNON.Vec3(v.x, v.y, v.z)
  })
  const faces: number[][] = (<Geometry>geometry).faces.map(function (f) {
    return [f.a, f.b, f.c]
  })
